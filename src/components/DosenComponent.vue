...
export default {
  ...
  methods: {
    simpan() {
      if (!this.name && !this.nidn) {
        alert("Data NAMA & NIDN Belum diisi");
        return false;
      }
      if (!this.nidn) {
        alert("Data NIDN Belum diisi");
        return false;
      }
      ...
      const dosen = new Dosen(this.name, this.nidn);
      this.submittedData.push(dosen);
      alert("Data Dosen (" + dosen.name + ") Berhasil disimpan");
      this.name = "";
      this.nidn = "";
    },
  },
